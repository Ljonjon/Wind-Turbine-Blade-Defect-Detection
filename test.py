...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', required=True, type=str,
                        help='Path to the trained YOLO weight file')
    ...
    model.val(
        data='datasets/blade2/data.yaml',  # Path to the dataset configuration file
        conf=0.001,                          # Confidence threshold for detections
        iou=0.5,                           # Intersection Over Union (IOU) threshold for non-max suppression
        split='test',                      # Dataset split to use for validation (e.g., 'test', 'val')
        imgsz=640,                         # Size of input images for validation
        plots = True 
    )